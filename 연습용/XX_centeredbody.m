...
lb=W*12/128;    % 70 or 120        when W=1280
tb=H*18/80;     % 180

recsize=[W*520/1280 H*175/800];     % W*450 or 520
    ...
    for j=1:numel(z)
        if ~barsize(5,j) == 0
            ...
            while(1)
                ...
                if button(1)                    
                    display_survey(z, seeds_i, target_i, words,'button');
...
response_W = Screen(theWindow, 'DrawText', double(words{target_i,seeds_i}), 0, 0);

x(1) = W/2 - interval/2 - response_W;
x(2) = W/2 + interval/2;
    while(1)
%         display_survey(z, seeds_i, target_i, words,'body');

        Screen(theWindow, 'FillRect', bgcolor, window_rect);
        Screen('PutImage', theWindow, bodymap); % put bodymap image on screen
        Screen('TextSize', theWindow, 30);
        DrawFormattedText(theWindow, double(words{target_i,seeds_i}), x(1), 80, white, [], [], [], 1.5);      
        Screen('TextSize', theWindow, 50);
        DrawFormattedText(theWindow, double(words{target_i+1,seeds_i}), x(2), 80, white, [], [], [], 1.5);
